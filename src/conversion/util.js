"use strict";
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = y[op[0] & 2 ? "return" : op[0] ? "throw" : "next"]) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [0, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var get_tree = function (expr_or_tree) {
    if (expr_or_tree === undefined || expr_or_tree === null)
        return undefined;
    var tree;
    if (expr_or_tree.tree !== undefined)
        tree = expr_or_tree.tree;
    else
        tree = expr_or_tree;
    return tree;
};
exports.get_tree = get_tree;
function subsets(arr, m) {
    var n, i, i, sub, _i, sub_1, val;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                n = arr.length;
                if (m === undefined)
                    m = n;
                if (m === 0)
                    return [2 /*return*/];
                i = 0;
                _a.label = 1;
            case 1:
                if (!(i < n)) return [3 /*break*/, 4];
                return [4 /*yield*/, [arr[i]]];
            case 2:
                _a.sent();
                _a.label = 3;
            case 3:
                i++;
                return [3 /*break*/, 1];
            case 4:
                if (m === 1)
                    return [2 /*return*/];
                i = 0;
                _a.label = 5;
            case 5:
                if (!(i < n)) return [3 /*break*/, 10];
                sub = subsets(arr.slice(i + 1), m - 1);
                _i = 0, sub_1 = sub;
                _a.label = 6;
            case 6:
                if (!(_i < sub_1.length)) return [3 /*break*/, 9];
                val = sub_1[_i];
                return [4 /*yield*/, [arr[i]].concat(val)];
            case 7:
                _a.sent();
                _a.label = 8;
            case 8:
                _i++;
                return [3 /*break*/, 6];
            case 9:
                i++;
                return [3 /*break*/, 5];
            case 10: return [2 /*return*/];
        }
    });
}
exports.subsets = subsets;
