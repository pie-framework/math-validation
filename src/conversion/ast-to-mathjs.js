...
var log = log_1.logger("mv:ast-to-math");
var m = mathjs_1.mathjs;
var operators = {
    "+": function (operands) {
...
var AstToMathJs = (function () {
    function AstToMathJs(_a) {
        var _b = (_a === void 0 ? {} : _a).mathjs, mathjs = _b === void 0 ? null : _b;
        // if (mathjs) node = mathjs.expression.node;
    }
    AstToMathJs.prototype.convert = function (tree) {
        ...
        if (operator === "apply") {
            ...
            var f = new m.SymbolNode(operands[0]);
            var args = operands[1];
            var f_args = void 0;
            if (args[0] === "tuple")
        ...
        if (operator === "lts" || operator === "gts") {
            ...
            var arg_nodes = args.slice(1).map(function (v, i) {
                return this.convert(v);
            }.bind(this));
            var comparisons = [];
            for (var i = 1; i < args.length - 1; i++) {
                if (strict[i]) {
                    if (operator === "lts")
                        comparisons.push(new m.OperatorNode("<", "smaller", arg_nodes.slice(i - 1, i + 1)));
                    else
                        comparisons.push(new m.OperatorNode(">", "larger", arg_nodes.slice(i - 1, i + 1)));
                }
                else {
                    if (operator === "lts")
                        comparisons.push(new m.OperatorNode("<=", "smallerEq", arg_nodes.slice(i - 1, i + 1)));
                    else
                        comparisons.push(new m.OperatorNode(">=", "largerEq", arg_nodes.slice(i - 1, i + 1)));
                }
            }
            var result = new m.OperatorNode("and", "and", comparisons.slice(0, 2));
            for (var i = 2; i < comparisons.length; i++)
                result = new m.OperatorNode("and", "and", [result, comparisons[i]]);
            return result;
        }
        if (operator === "=") {
            var arg_nodes = operands.map(function (v, i) {
                return this.convert(v);
            }.bind(this));
            var comparisons = [];
            for (var i = 1; i < arg_nodes.length; i++) {
                comparisons.push(new m.OperatorNode("==", "equal", arg_nodes.slice(i - 1, i + 1)));
            }
        ...
        if (operator === "in" ||
            ...
            var args = interval[1];
            var closed_1 = interval[2];
            if (args[0] !== "tuple" || closed_1[0] !== "tuple")
                throw new Error("Badly formed ast");
            ...
            var result = new m.OperatorNode("and", "and", comparisons);
